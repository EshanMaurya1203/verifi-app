...
function badgeClassName(type: PaymentMethod["badge"]) {
  if (type === "API Verified") {
    return "border border-[rgba(185,255,75,0.2)] bg-[#0d1f00] text-[#b9ff4b]";
  }
  return "";
}
...
export default function SubmitPage() {
  ...
  const inputClass =
    "h-11 w-full rounded-lg border border-[#1e1e1e] bg-[#161616] px-4 text-[14px] text-[#edede9] placeholder:text-[#333333] outline-none transition-colors duration-150 focus:border-[#b9ff4b]";
  const labelClass =
    "mb-1.5 block text-[11px] font-medium uppercase tracking-[0.8px] text-[#606060]";
  const sectionTitleClass =
    "mb-4 border-b border-[#1a1a1a] pb-2 text-[11px] uppercase tracking-[1.5px] text-[#333333]";
  ...
  const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch("/api/startup-submissions", {
        method: "POST",
      ...
      setIsSuccess(true);
      if (typeof data.slot_number === "number") {
  ...
  return (
    <div className="min-h-screen bg-[#080808] pt-20 text-[#edede9]">
      <Navbar />

      <header className="mx-auto max-w-[640px] px-6 pt-12 text-center">
        <h1 className="font-syne text-[44px] font-extrabold tracking-[-1.5px] text-[#edede9]">
          List your startup on Verifi — <span className="text-[#b9ff4b]">free</span>
        </h1>
        <p className="mt-3 text-[16px] font-light text-[#606060]">
          Get verified and join the most transparent startup revenue database.
        </p>
      </header>

      <section className="mx-auto mt-8 max-w-[640px] px-6">
        <div className="rounded-xl border border-[rgba(245,166,35,0.2)] bg-[#0f0f0f] px-5 py-4">
          <div className="flex items-center justify-between gap-3">
            <div className="text-[13px] text-[#a0a09a]">
          ...
          <div className="mt-2.5 h-[3px] rounded-full bg-[#1e1e1e]">
            <div
      ...
      <main className="mx-auto max-w-[640px] px-6 pb-20">
        <div className="relative mt-0 overflow-hidden rounded-2xl border border-[#1e1e1e] bg-[#0f0f0f] p-6 md:p-10">
          ...
          {isSuccess ? (
            ...
            >
              <div className="flex h-16 w-16 items-center justify-center rounded-full border border-[rgba(185,255,75,0.3)] bg-[#0d1f00]">
                <Check className="h-7 w-7 text-[#b9ff4b]" />
              </div>
              <h2 className="mt-6 font-syne text-[32px] font-extrabold">You&apos;re in!</h2>
              <div className="mt-3 inline-flex rounded-lg border border-[rgba(185,255,75,0.3)] bg-[#0d1f00] px-4 py-2 font-syne text-[16px] font-bold text-[#b9ff4b]">
                Founding Member #{slotNumber ?? claimedCount + 1}
              </div>
              <p className="mt-4 max-w-[520px] text-[14px] text-[#a0a09a]">
              ...
              <Link
                ...
                target="_blank"
                className="mt-6 rounded-xl border border-[#262626] bg-[#161616] px-4 py-2 text-sm text-[#edede9] transition-colors hover:border-[#b9ff4b]"
              >
                Share on Twitter
              </Link>
            </motion.div>
            ...
            <form onSubmit={onSubmit} noValidate>
              <div className="mb-6">
                <div className="mb-2 flex items-center justify-between text-[12px] text-[#606060]">
                  <span>Step {step} of 4</span>
                  <span>{Math.round(stepProgressPercentage)}%</span>
                </div>
                <div className="h-2 rounded-full bg-[#1e1e1e]">
                  <div
                    className="h-2 rounded-full bg-[#b9ff4b] transition-all duration-300"
                  ...
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                      <label className={labelClass}>
                        Full name <span className="text-[#b9ff4b]">*</span>
                      </label>
                      <input
                        className={`${inputClass} ${errors.fullName ? "border-[#ff4b4b]" : ""}`}
                        value={form.fullName}
                    ...
                    <div>
                      <label className={labelClass}>
                        Email <span className="text-[#b9ff4b]">*</span>
                      </label>
                      <input
                        type="email"
                        className={`${inputClass} ${errors.email ? "border-[#ff4b4b]" : ""}`}
                        value={form.email}
              ...
              {step === 2 && (
                <section>
                  ...
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                      <label className={labelClass}>
                        ...
                        <span className="text-[#b9ff4b]">*</span>
                      </label>
                      <input
                        className={`${inputClass} ${errors.startupName ? "border-[#ff4b4b]" : ""}`}
                  ...
                  <div className="mt-4">
                    <label className={labelClass}>
                      Business type <span className="text-[#b9ff4b]">*</span>
                    </label>
                    <div className="relative">
                      <select
                        className={`${inputClass} appearance-none pr-10 ${errors.businessType ? "border-[#ff4b4b]" : ""}`}
                      ...
                      <ChevronDown className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#606060]" />
                    </div>
                    {errors.businessType ? (
                ...
                <section>
                  ...
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                      ...
                      <input
                        className={`${inputClass} ${errors.mrr ? "border-[#ff4b4b]" : ""}`}
                        value={form.mrr}
                    ...
                    <div>
                      ...
                      <input
                        className={`${inputClass} ${errors.arr ? "border-[#ff4b4b]" : ""}`}
                        value={form.arr}
                  ...
                  <div className="mt-6">
                    <p className="mb-3 text-[12px] text-[#606060]">
                          ...
                          <button
                            ...
                            className={`flex items-center gap-3 rounded-lg border p-3 text-left transition-all duration-150 ${
                              isChecked
                                ...
                                : "border-[#1e1e1e] bg-[#161616] hover:border-[#333333]"
                            }`}
                          >
                            <input
                              type="checkbox"
                            ...
                            <span
                              className={`flex h-4 w-4 items-center justify-center rounded-sm border text-[10px] ${
                                isChecked
                                  ? "border-[#b9ff4b] bg-[#b9ff4b] text-[#080808]"
                                  : "border-[#333333] bg-[#0f0f0f] text-transparent"
                              }`}
                            >
                              ✓
                            </span>
                            <span className="flex-1 text-[13px] text-[#a0a09a]">
                              {item.label}
                            </span>
                            <span
              ...
              {step === 4 && (
                <section>
                  ...
                  <div className="mt-4">
                    <label className={labelClass}>
                      City / Country <span className="text-[#b9ff4b]">*</span>
                    </label>
                    <input
                      className={`${inputClass} ${errors.cityCountry ? "border-[#ff4b4b]" : ""}`}
                  ...
                  <div className="mt-6">
                    ...
                    <textarea
                      rows={4}
                      className="w-full rounded-lg border border-[#1e1e1e] bg-[#161616] px-4 py-3 text-[14px] text-[#edede9] placeholder:text-[#333333] outline-none transition-colors duration-150 focus:border-[#b9ff4b]"
              ...
              {submitError ? (
                <div className="mt-6 rounded-lg border border-[#ff4b4b] bg-[#1a0000] p-3 text-sm text-[#ff4b4b]">
                  {submitError}
                </div>
              ) : null}
              ...
              <div className="mt-8 flex items-center gap-3">
                {step > 1 ? (
                  <button
                    ...
                    className="h-[52px] rounded-xl border border-[#2a2a2a] px-6 text-[14px] text-[#a0a09a] transition-colors hover:border-[#3a3a3a] hover:text-[#edede9]"
                  >
                    Back
                  </button>
                ) : null}
                ...
                {step < 4 ? (
                  <button
                    ...
                    className="flex h-[52px] flex-1 items-center justify-center rounded-xl bg-[#b9ff4b] font-syne text-[16px] font-bold text-[#080808] transition-colors hover:bg-[#a8e630]"
                  >
                    Next
                  </button>
                ) : (
                  <button
                    ...
                    className="flex h-[52px] flex-1 items-center justify-center gap-2 rounded-xl bg-[#b9ff4b] font-syne text-[16px] font-bold text-[#080808] transition-colors hover:bg-[#a8e630] active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-80"
                  >
                    {isSubmitting ? (
                      <>
                        <span className="h-5 w-5 animate-spin rounded-full border-2 border-[#080808] border-t-transparent" />
                        Submitting...
                      </>
                    ) : (
              ...
              <p className="mt-3 text-center text-[11px] text-[#333333]">
