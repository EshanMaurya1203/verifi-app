 ...
 *
 * Analyzes payment records to determine whether a startup's payment history
 * looks consistent or contains synthetic patterns.
 *
 * Input:  Array of { amount, timestamp } from the revenue_events table
 * Output: { consistency_score, consistency_flags, consistency_level }
 *
...
export type ConsistencyLevel = "Refining" | "Moderate" | "Consistent";
...
/**
 * Analyzes an array of revenue events and returns a consistency assessment.
 */
export function analyzeRevenueConsistency(events: RevenueEvent[]): ConsistencyResult {
  // ── Edge case: insufficient data ──────────────────────────
  if (!events || events.length < MIN_EVENTS_FOR_ANALYSIS) {
    return {
      consistency_score: 50,
      consistency_flags: events?.length
  ...
  const level = getConsistencyLevel(finalScore);

  return {
    consistency_score: finalScore,
    consistency_flags: flags.slice(0, 6), // Keep concise
    consistency_level: level,
  };
}

function getConsistencyLevel(score: number): ConsistencyLevel {
  if (score < 30) return "Refining";
